...
The *forests* data set has 32 features and is a multipolygon along with the *parks* data set, which has 47 features. The *playgrounds* data set, which has 51 features, is a point along with the *shelters* data set, which has 19 features.

2. What is the CRS value in these objects?

The CRS values for the *forests*, *parks* and *shelters* data sets are "ETRS89 / UTM zone 32N". The *playgrounds* data have no CRS value.
...
Using the `dim` function shows that the `elevation` raster has the dimensions: 396 876 1

2. What is the resolution of `aarhus` orthophoto? How many layers does it contain and what do they represent?

The resolution is 1x1 and `aarhus` has 5 nlayers. The smaller the pixel size the finer the spatial resolution.


Now you've learned how to read in single and multi-band rasters. You should have noticed, based on the `nlayers()` function, that the `elevation` object has a single layer and the `aarhus` object has four (it shows five for some reason?).
...
3. It makes sense that the extents and number of cells for Danish national elevation data and Aarhus city would be different because their sizes diverge, but why are the units so different? What are these units?*

The `elevation` data dont show any units? The `aarhus` photo uses meters as unit.
...
crs_2 <- "+proj=utm +zone=32 +ellps=WGS84 +units=m +no_defs"
crs(aarhus) <- crs_2
...
playgrounds_crs <- st_transform(playgrounds, crs = 4326)

# Project elevation to match the CRS of aarhus
elevation_crs <- projectRaster(elevation, crs = 4326, method = "ngb")
