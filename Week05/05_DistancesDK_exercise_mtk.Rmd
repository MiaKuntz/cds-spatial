...
distCosine(my_points) # the same as the Haversine distance
```

### Meeus distance
```{r meeus}
 # Calculate Meeus distance
distMeeus(my_points) # this is the shortest                
```
...
```{r rhumb}
distRhumb(my_points)  # this is longer than the above  
```
...
distVincentySphere(my_points) # same as Haversine and Cosines
...
The shortest is the Meeus and the longest is the rhumb line.

```{r diff}
# Calculate the difference in km
(distRhumb(my_points)-distMeeus(my_points))/1000 # 711 km
(16860264-16148937)/1000

# Calculate percent difference 
(16860264-16148937)/16860264*100 # 4,21%
```
...
# difference
euc_dist-geo_dist # 5876710 meters or circa 5876 km
...
monuments <- read_sf("../../large_data/fortidsminder/anlaeg_all_25832.shp")
viking <- monuments %>% 
...
# Plot the results
plot(nb_25832$geometry, border = "blue")
