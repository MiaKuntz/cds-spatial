...
my_token <- "pk.eyJ1IjoibWlha3VudHoiLCJhIjoiY2xld3FqejJ2MWM5aTN5czBtMnZjNzFkdSJ9.I2dnbv1NHqZKxW5s-taUEQ"
...
hjem <- mb_geocode("Ny Munkegade 79, Aarhus, 8000, Denmark")
hjem

fam <- mb_geocode("Skejbygårdsvej 318, Risskov, 8240, Denmark")
fam

london_hjem <- mb_geocode("Oaklands, Argyle Rd, Ealing, W13 0HG, England")
london_hjem
```
...
nobel_iso_drive
# this generates three polygons from largest to smallest
```
...
route_fam <- mb_directions(origin = hjem,
                       destination = fam,
                       profile = "walking")

mapbox_map %>%
  addPolylines(data = route_fam, 
               popup = paste0(
                 "Distance (km): ",
                 round(route$distance, 1), 
                 "<br/>Time (minutes): ",
...
route_dir <- mb_directions(origin = hjem,
                       destination = fam,
                       profile = "walking",
                           steps = TRUE)
route_dir
...
Sys.setenv(OPENCAGE_KEY = "2c63769352cb4b73b39f3375d2696a72")
...
```{r}
getData("GADM", country = "DNK", level = 1)

# Something like this??
#st_area(Midtjylland) <- st_area(st_union(driving_isos))
#st_difference(Midtjylland(st_union(driving_isos)))
```
