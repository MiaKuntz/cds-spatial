...
The `cities_sf3035` uses meters as it's unit of measurement
...
plot((cities_5km$geometry)[1:10], col = "yellow")
plot(st_geometry(cities_sf3035)[1:10], pch=20, cex = 0.1, add = TRUE)
# I am not sure on how to plot these properly as we have not worked a lot with plotting buffers - looking forward to seeing a solution! :)
...
tm_shape(ten_buffers)  +
  ...
  tm_dots(col = "black", 
             size = 0.1) +
 tm_scale_bar(breaks = c(0, 10, 20, 30, 40),
               text.size = 0.5,
...
The map shows the buffers actually do span 10km across, although it does not show much else.
...
As can be seen from the above map, the "view" mode drops the main title of the map, the compass, the credits, and the locked scale bar (as the map is now interactive).
...
download.file("https://sciencedata.dk/public/b6b6afdb969d378b70929e86e58ad975/EDH_subset_2021-02-15.json", "../../large_data/inscriptions.json")

# Load it into R from wherever you put it, and convert into a tibble
list_json <- jsonlite::fromJSON("../../large_data/inscriptions.json")
