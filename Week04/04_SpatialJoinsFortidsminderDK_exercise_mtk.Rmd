...
municipalities_25832 <- municipalities %>% 
  ...
  st_transform(municipalities, crs=25832)
...
nb_buf <- nb %>% 
  ...
  st_transform(crs = 25832) %>% # and then project the right crs
  st_buffer(10000)
...
# Get the indexes of which viking monuments intersect and which units are contained by the buffer
int <- viking_int[[1]]
cont <- parks_cont[[1]]
...
We defined the fully contained parks as a new 

3. How many parks and viking monuments respectively are there within the buffer?
There are 79 viking monuments and 32 parks within the buffer
