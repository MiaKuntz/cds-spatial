...
wc <- read_sheet("https://docs.google.com/spreadsheets/d/1iFvycp6M6bF8GBkGjA2Yde2yCIhiy5_slAkGF-RUF7w/edit#gid=0", col_types = "cnnnnnnnn")
...
They were called differently back then, so all which share suburb_id used to be the same suburb
...
```{r merge data}
wc_spatial <- suburbs %>% 
  merge(wcdata, by.x= "id",by.y ="suburb_id") %>% # defining what the shared columns are names in each dataset
    st_make_valid() # makes all polygons valid
wc_spatial
sf::st_is_valid(wc_spatial) # checking if all polygons are valid 
```
...
# filtering out anything but amenities and transforming into local utm and selecting name
rpoint <- bath_uniq$osm_points %>% 
  filter(!is.na(amenity)) %>% 
  st_transform(32632) %>%
  dplyr::select(name) 

# transforming polygons and creating centroids
rpoly  <- bath_uniq$osm_polygons %>% 
  st_transform(32632) %>% 
  dplyr::select(name)  %>% st_centroid()

# binding them together (attaching two datasets)
baths_osm <- rbind(rpoly,rpoint)   

# intersecting them and unioning into one multipoint
...
baths_cph <- wc_spatial%>% 
  st_centroid() %>% #here I am fixing a duplicate vertex
  st_transform(32632) %>% 
...
```{r get-hist-baths}
#baths <- read_sheet("https://docs.google.com/spreadsheets/d/15i17dqdsRYv6tdboZIlxTmhdcaN-JtgySMXIXwb5WfE/edit
#gid=0",
