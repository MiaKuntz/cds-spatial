...
# verify crs match with elevation
elevation
mounds

st_crs(elevation)==crs(mounds) # should be TRUE

# create bounding box
mounds_bb <- st_make_grid(mounds, n = 1)

# plot mounds and mounds_bb together
plot(st_geometry(mounds), col = "black", border = "white")
...
plot(elev)
plot(st_geometry(mounds), col = "black", border = "white", add = TRUE)
plot(mounds_bb, add = TRUE)
...
areas <- st_area(survey) # the units are in square meters
...
east_small <- aggregate(east, fact = 10, mean)
